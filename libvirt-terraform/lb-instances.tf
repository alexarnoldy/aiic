...
data "template_file" "haproxy_apiserver_backends_primary" {
  count    = var.primarys
  template = "server $${fqdn} $${ip}:6443\n"

  vars = {
    fqdn = "${var.stack_name}-primary-${count.index}.${var.dns_domain}"
    ip   = cidrhost(var.network_cidr, 512 + count.index)
  }
}

data "template_file" "haproxy_gangway_backends_primary" {
  count    = var.primarys
...
data "template_file" "haproxy_dex_backends_primary" {
  count    = var.primarys
...
data "template_file" "lb_haproxy_cfg" {
  ...
  vars = {
    apiserver_backends = join(
      "  ",
      data.template_file.haproxy_apiserver_backends_primary.*.rendered,
    )
    gangway_backends = join(
      "  ",
      data.template_file.haproxy_gangway_backends_primary.*.rendered,
    )
    dex_backends = join(
      "  ",
      data.template_file.haproxy_dex_backends_primary.*.rendered,
    )
  }
}
