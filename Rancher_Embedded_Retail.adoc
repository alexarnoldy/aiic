...
This design was specifically created for providers of IT infrastructure software and hardware designed for retail outlets with no on-site IT support. This design emphasizes security and ease of use for the on-site staff. The two primary components of the installation are the highly available installation of SUSE Rancher and the Point-of-Service units that are pre-installed with a hardened and standardized image of SLE Micro with a K3s configured as a single-node cluster. 
